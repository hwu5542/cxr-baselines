...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("cnn_rnn_bert.log"), logging.StreamHandler()],
)
...
class CNN_RNN_BERT_Model(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        decoder_output, _ = self.decoder(
            (decoder_input, image_features), training=training
        )
...
def beam_search_decode(model, features, beam_width=4, max_length=MAX_SEQ_LENGTH):
    ...
    for _ in range(max_length):
        ...
        for seq, score, state in beams:
            ...
            predictions, new_state = model.decoder(
                (decoder_input, image_features), initial_state=state
            )
...
def load_and_prepare_data():
    ...
    train_features = np.stack(
        train_df["pooled_features"].apply(lambda x: np.array(x, dtype=np.float32))
    )
    test_features = np.stack(
        test_df["pooled_features"].apply(lambda x: np.array(x, dtype=np.float32))
    )
...
def train_and_predict():
    try:
        ...
        logging.info(
            f"Hardware: {'GPU' if gpu_devices else 'CPU'} ({len(gpu_devices)} GPUs)"
        )
        if gpu_devices:
            for i, gpu in enumerate(gpu_devices):
                logging.info(f"  GPU {i}: {tf.test.gpu_device_name()}")

        # Track runtime and GPU hours
        ...
        train_dataset = create_dataset(
            train_features, train_input_ids, train_attention_mask
        )
        test_dataset = create_dataset(
            test_features, test_input_ids, test_attention_mask
        )
        ...
        for epoch in range(EPOCHS):
            ...
            if (epoch + 1) % 8 == 0 or epoch == EPOCHS - 1:
                ...
                for i in tqdm(range(len(test_df)), desc="Generating predictions"):
                    ...
                    generated_report = beam_search_decode(
                        model, features, beam_width=BEAM_WIDTH
                    )
        ...
        total_gpu_hours = (total_time * len(gpu_devices)) / 3600 if gpu_devices else 0

        logging.info("\n--- Training Summary ---")
        logging.info(f"Total epochs: {EPOCHS}")
        logging.info(f"Avg time/epoch: {timedelta(seconds=int(avg_epoch_time))}")
        logging.info(f"Total runtime: {timedelta(seconds=int(total_time))}")
        logging.info(
            f"GPU hours used: {total_gpu_hours:.2f} (on {len(gpu_devices)} GPUs)"
        )
