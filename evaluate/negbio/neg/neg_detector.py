...
def is_neg_graph1(graph):
    # no XXX
    # resolution of XXX
    if 'T0' in graph.node and graph.nodes['T0']['lemma'] in ['no', 'resolution', 'resolved']:
        # no verb
...
def is_neg_graph2(graph, begin, end):
    ...
    state = 0
    # sort nodes
    for node in sorted(graph.nodes(), key=lambda n: graph.nodes[n]['start']):
        if graph.nodes[node]['end'] > end:
            break

        if state == 0:
            if graph.nodes[node]['lemma'] in (
